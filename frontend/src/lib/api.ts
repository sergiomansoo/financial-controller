...
import type {
  ApiProblem, AssistantChatRequest, AssistantChatResponse, AuthCredentials, AuthResponse, Budget, Category,
  CategoryRule, CategoryRuleApplyResponse, DashboardData, ImportHistoryItem, ImportPreview, ImportResponse,
  ManualTransactionInput, RegistrationDetails, SavingsGoal, SavingsGoalInput, Transaction, TransactionPage,
  TransactionQuery, TransactionTotal, TransactionTotalQuery,
} from '../types/api'
...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new ApiError(response.status, problem?.code, problem?.message ?? `Request failed with status ${response.status}`)
  }
  return payload as T
}

export function login(credentials: AuthCredentials) { return request<AuthResponse>('/auth/login', { method: 'POST', body: JSON.stringify(credentials) }) }
export function register(details: RegistrationDetails) { return request<AuthResponse>('/auth/register', { method: 'POST', body: JSON.stringify(details) }) }
export function uploadStatement(file: File) { const body = new FormData(); body.append('file', file); return request<ImportResponse>('/imports', { method: 'POST', body }) }
export function getImports() { return request<ImportHistoryItem[]>('/imports') }
export function getTransactions(month: string) { return request<Transaction[]>(`/transactions?month=${encodeURIComponent(month)}`) }
...
export function getCategories() { return request<Category[]>('/categories') }
export function updateTransactionCategory(id: string | number, categoryId: string | number, learn = true) { return request<Transaction>(`/transactions/${id}/category`, { method: 'PATCH', body: JSON.stringify({ categoryId, learn }) }) }
...
export function askAssistant(input: AssistantChatRequest) { return request<AssistantChatResponse>('/assistant/chat', { method: 'POST', body: JSON.stringify(input) }) }
